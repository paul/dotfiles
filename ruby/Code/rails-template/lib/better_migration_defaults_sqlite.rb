...
module BetterMigrationDefaults
  ...
  # Pick some better defauls for `references`:
  #  - on_delete: :cascade
  #  - null: false
  ...
  module BetterTimeouts
    ...
    #
    # Handled by StrongMigrations config
    # def migrate(...)
  ...
  module SafeAddUniqueIndex
    def safe_add_unique_index(table_name, column_name, **options)
      ...
      when false # index exists, but is invalid. Try re-indexing it
        # If it fails again, this leaves behind indexes like `my_idx_ccnew1`
        # connection.execute(%(REINDEX INDEX CONCURRENTLY "#{name}"))
  ...
  refine ActiveRecord::ConnectionAdapters::SQLite3::TableDefinition do
