---
name: reek
description: Use when fixing or evaluating Reek warnings in Ruby projects — explains how to treat smells as design feedback, choose refactorings, and avoid suppression-first fixes.
---
...
1. Read the smell name and the flagged method or class.
2. Identify the design pressure:
   - too many responsibilities
   - caller-controlled branching
   - missing abstraction
   - behavior living on the wrong object
   - hidden state or implicit setup
3. Fix the smallest real design problem.
4. Re-run Reek on the changed files using `reek .` (always pass the explicit directory argument — bare `reek` defaults to scanning `.` only when stdin is a TTY, which is not the case in agent runners or CI).
5. If the warning appears to conflict with an intentional framework pattern, stop and check project-local guidance before changing code or suppressing the smell.

## Principles

- Do not suppress a smell just to make the warning disappear.
- Prefer moving behavior to a better object over extracting helpers mechanically.
- Prefer fewer, clearer public methods.
- Prefer explicit initialization over hidden instance-variable setup.
- If the refactor would substantially change architecture, stop and ask.
...
A method has more than N statements (default is 5). Each `if`, `else`,
...
- Usually whitelist `_` (ignored parameter). All other single-letter or numbered
