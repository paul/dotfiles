...
When running `git add` and `git commit` together, avoid tight chains that can trip over transient
`index.lock` contention from editors, watchers, or another agent. Use separate commands or insert
`sleep 0.1` between staging and committing. If Git reports `index.lock` exists, retry after a short
sleep before investigating or asking to remove the lock.
